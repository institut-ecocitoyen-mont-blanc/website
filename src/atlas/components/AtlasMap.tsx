...
import AtlasDetailDialog from "@/atlas/components/AtlasDetailDialog";
import AtlasWaterDetails from "@/atlas/components/AtlasWaterDetails";
import AtlasDrinkingDetails from "@/atlas/components/AtlasDrinkingDetails";
import AtlasBathingDetails from "@/atlas/components/AtlasBathingDetails";
import { bathingQuality, bathingSamples, type BathingData, type BathingPoint } from "@/atlas/lib/bathing-water";
import AtlasInventoryDetails from "@/atlas/components/AtlasInventoryDetails";
import { environmentalPin } from "@/atlas/components/EnvironmentalPin";
import { useDrinkingQuality } from "@/atlas/components/WaterQuality";
import { unknownQuality } from "@/atlas/lib/environmental-quality";
import { riverQuality, type RiverAssessments } from "@/atlas/lib/river-assessments";
...
import { LAYERS, type LayerId, type LayerPoint } from "@/atlas/lib/environmental-layers";
import { loadPublishedData } from "@/atlas/lib/published-data";
import { CCPMB_BOUNDS as AREA, filterCcpmbPoints } from "@/atlas/lib/ccpmb-territory";
...
function useRemoteLayer(id: Exclude<RemoteId, "atmo">, enabled: boolean, revision: number, snapshot?: RiverCatalogue) {
  ...
  useEffect(() => {
    ...
    loadPublishedData<RiverCatalogue>(id, controller.signal).then((data) => {
      if (!controller.signal.aborted) setState({ points: filterCcpmbPoints(data.points), status: "ready", fetchedAt: data.fetchedAt ?? undefined });
    }).catch((error: unknown) => {
